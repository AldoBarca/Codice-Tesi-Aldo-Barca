...
daddi $s6,$0,0 ;  numero elementi non nulli di un canale, channelsize[channel]
daddi $s0,$0,0 ; channel=0 ,channel in s0  variabile per il canale dell'img



for_canale: slti $t0,$s0,8191 ; è letterlamente i<nChannel=4 dove ogni canale dista 2048 byte l'uno dall'altro e dunque si faranno 4 iterazioni
...
dsll $s4,$s3,3    ;pos=first*8 , in pratica è solo per allineare gli accessi in memoria
...
dadd $t9,$s2,$s1  ; calcolo in $t9 x+y della conv x
ld $t8, riga1_conv($t9) ; carico in $t8 conv[x][y]  
ld $t3,Px($s4) ;t3=px[pos]   ;problema ho un numero devo moltiplicarlo per la riga.  è 0,1 o 2
...
dadd $t6,$t8,$t7 ;calcolo conv+ il prodotto come nell'algoritmo in c 
...
daddi $s6,$s6,8  ; channelsize di uno specifico canale
